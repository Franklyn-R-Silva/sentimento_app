...
class FlutterFlowDynamicModels<T extends FlutterFlowModel> {
  ...
  List<S> getValues<S>(final S? Function(T) getValue) {
    return _childrenIndexes.entries
        ...
        .map((final e) => getValue(_childrenModels[e.key]!) ?? _getDefaultValue<S>()!)
        .toList();
  }

  S? getValueAtIndex<S>(final int index, final S? Function(T) getValue) {
    final uniqueKey = _childrenIndexes.entries.firstWhereOrNull((final e) => e.value == index)?.key;
  ...
  void dispose() => _childrenModels.values.forEach((final model) => model.dispose());
  ...
  void _updateActiveKeys(final String uniqueKey) {
    ...
    if (shouldResetActiveKeys) {
      ...
      SchedulerBinding.instance.addPostFrameCallback((final _) {
        _childrenIndexes.removeWhere((final k, final _) => !_activeKeys!.contains(k));
